...
def CalculationFRETmetric(impD_,impA_, FRETmetric_):
	if FRETmetric_ == FRETmetrics[0]:
		impD_ = IC().run("Add create 32-bit", impD_,impA_)#------- 1) image Donor+Acceptor -> Denominator
		...
		impA_ = IC().run("Divide create 32-bit", impA_, impD_)#------- 2) image Acceptor/(Donor+Acceptor)
...
def CalculationFRETratio(imp1_,imp2_) : #image imp2_/imp1_
	...
	IJ.run(imp1_, "NaN Background", "")	
	impRatio_ = IC().run("Divide create 32-bit", imp2_, imp1_)
	IJ.run(impRatio_, "NaN Background", "")	
	IJ.run(impRatio_, "Enhance Contrast", "saturated=0.35")
	return impRatio_
