...
def subtractBG(imp_, roi_):
	...
	mean = imp_.getStatistics(Measurements.MEAN).mean
	IJ.run(imp_, "Select None", "")
	IJ.run(imp_, "Subtract...", "value="+str(mean)+" slice")
	return mean	
...
for slic in range(nbSlice): 
	...
	applyThreshold(impDonor_slice, 0,  maxVal - 1)
	applyThreshold(impAcceptor_slice, 0,  maxVal - 1)
