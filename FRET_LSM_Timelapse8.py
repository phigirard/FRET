...
def getBackgroundROI(imp_,) :
	...
	imp_.show()
	IJ.setTool("rectangle")
	myWait = WaitForUserDialog("Select a ROI", "Select a ROI for background substraction or photobleaching correction")
	myWait.show()
	roi_  = imp_.getRoi()
	imp_.killRoi
	imp_.hide()
...
nbSlice = impDonor.getStackSize()
if nbSlice > 1:
	IJ.run(impDonor, "Smooth", "stack")
	IJ.run(impAcceptor, "Smooth", "stack")
else :
	IJ.run(impDonor, "Smooth", "")
	IJ.run(impAcceptor, "Smooth", "")
